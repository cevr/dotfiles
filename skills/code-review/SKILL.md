...
Systematic audit and cleanup. Not a style guide — that's `code-style`.
...
| Type shortcuts | `any`, `as unknown as X`, `!`, `@ts-ignore` |
...
```bash
git diff --name-only main...HEAD | xargs grep -n \
  "// TODO\|// FIXME\|// eslint-disable\|as any\|as unknown\|@ts-ignore\|console\.log\|console\.debug" \
  2>/dev/null
```
...
```bash
repo fetch owner/repo          # or npm:package@version
repo path -q owner/repo        # then grep/read the source
```
...
**Source reference**: [If comparing against upstream: `~/.cache/repo/org/lib/src/example.ts:30`]
