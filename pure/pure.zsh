...
prompt_pure_preprompt_render() {
	...
	# Timestamp (static per prompt cycle to avoid async redraw ghosts).
	preprompt_parts+=('%F{$prompt_pure_colors[timestamp]}${prompt_pure_timestamp}%f')
	...
	# Strip terminal shell-integration marks (e.g. Ghostty/Kitty OSC 133)
	# injected into PS1 by terminal precmd hooks. These break
	# prompt_newline matching when preexec doesn't fire (empty Enter).
	local stripped_prompt=$PROMPT
	stripped_prompt=${stripped_prompt//$'%{\e]133;A;cl=line\a%}'}
	stripped_prompt=${stripped_prompt//$'%{\e]133;A;k=s\a%}'}
	stripped_prompt=${stripped_prompt//$'%{\e]133;P;k=s\a%}'}
	stripped_prompt=${stripped_prompt//$'%{\e]133;B\a%}'}
	if [[ $stripped_prompt = *$prompt_newline* ]]; then
		# Remove everything from the prompt until the newline. This
		# removes the preprompt and only the original PROMPT remains.
		cleaned_ps1=${stripped_prompt##*${prompt_newline}}
	fi
...
prompt_pure_precmd() {
	...
	# Capture timestamp once so async redraws don't see a changed prompt.
	typeset -g prompt_pure_timestamp=${(%):-%*}
